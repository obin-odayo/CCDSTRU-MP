...
/*
  NextPlayerMove: checks if pos is in F3 (set of all available moves). If yes,    updates the current player's set F.

  @params:
  - pos [struct orderedPair]: current player's move.
  - F3 [struct orderedPair]: set of all available moves.
  - sizeF3 [int]: size of F3.
  - FPlayer[struct orderedPair]: set of the current player's moves.
  - sizeFPlayer [int*]: size of FPlayer.
  
  @returns:
  - 1 if pos is in F3 and was able to successfully update player's F set, else    returns 0.

  Note: make sure to run systemFact1 right after calling this function.
  
  Author: Kraut
*/
int NextPlayerMove(struct orderedPair pos, struct orderedPair F3[], int sizeF3, struct orderedPair FPlayer[], int * sizeFPlayer)
{
  int i;
  int ind;
...
int main()
{
 ...
 {6, 6},
  ...
  int sizeF1 = 10;
  struct orderedPair F2[10] = {{2, 1}, {2, 2}, {2, 3}, {2, 5}};
  int sizeF2 = 5;
  ...
  printf("%d\n", NextPlayerMove(pos, F3, sizeF3, F1, &sizeF1));
  sizeF3 = systemFact1(F1, F2, F3, sizeF1, sizeF2, sizeF3);
